...
function playlist(name,client,arg)
    ...
    if name == "search" then
        client:append("+----[ End of search - Use `search' to resetֲ ]")
    else
...
function get_time(var)
	...
	if input then
	    client:append(math.floor(vlc.var.get( input, var )/1000))	
	else
	    client:append("")
	end
    end
...
function seek(name,client,value)
    local input = vlc.object.input()
	if input then
	    vlc.var.set( input, "time" , value *1000)
		client:append(value)
	end
end
...
--[[ֲ Declare commands, register their callback functions and provide
     help strings here.
     Syntax is:
